...
pics %>%
  ...
  select(type, wgt_avg_resistance, avg_resistance,skew, Pokemon, num_4x,
         num_weaknesses, num_resists, num_doub_resist) %>%
  ...
  rename(
    ...
    `Meta Skew` = skew, 
    `No. 4x Weaknesses` = num_4x,
    `Number of Weaknesses` = num_weaknesses,
    `Number of Resists` = num_resists,
    `Number of .25 Resists` = num_doub_resist
  ) %>%
  formattable() %>%
...
* Reverse sort this and look how much the meta has punished Coalossal.  Everyone hates you, Coalossal!  
...
On the flip side, the high negative values indicate the meta has opened up to allow better than average viability for other Pokemon.  Salamence, Rayquaaza, Hawlucha, the fire/flying types may have openings.  Whether these Pokemon have the other factors needed to work themselves into the meta remains to be seen.  
